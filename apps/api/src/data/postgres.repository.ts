...
export class PostgresRepository implements CoreRepository {
  ...
  async listAllProperties() { const result = await getPool().query<{ id: string }>('SELECT id FROM properties ORDER BY updated_at DESC'); return Promise.all(result.rows.map(async (row) => (await this.findPropertyDetails(row.id))!)); }
